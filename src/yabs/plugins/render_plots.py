...
def render(context, options, plot_name, plot_dict):
	...
	scripts_path = os.path.relpath(context[KEY_OUT][KEY_SCRIPTS], context[KEY_OUT][KEY_ROOT])
	if scripts_path not in ['', '.']:
		scripts_path += '/'
	styles_path = os.path.relpath(context[KEY_OUT][KEY_STYLES], context[KEY_OUT][KEY_ROOT])
	if styles_path not in ['', '.']:
		styles_path += '/'

	for language_id in plot_languages.keys():

		plot_id = get_plotly_id(plot_languages[language_id][KEY_HTML]) if plot_type == KEY_PLOTLY else ''

		plot_out = context[KEY_TEMPLATES]['%s_%s' % (KEY_PLOT, plot_type)].render(
			**{
				KEY_SCRIPTS: scripts_path,
				KEY_STYLES: styles_path,
				KEY_ID: plot_id
				},
		...
		with open(os.path.join(context[KEY_OUT][KEY_ROOT], '%s_%s_%s.htm' % (KEY_PLOT, plot_name, language_id)), 'w') as f:
			f.write(plot_out)
