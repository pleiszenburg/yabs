...
LIB_DICT = {
    ...
    "jquery": {
        ...
        KEY_SRC: ["https://code.jquery.com/jquery-%s.min.js"],
    },
    "normalize.css": {
...
def fetch_library(context, library):
    ...
    with open(
        os.path.join(context[KEY_SRC][KEY_LIBRARIES], library, VERSION_FN), "r"
    ) as f:
    ...
    for ext in ["css", "js"] + FONT_SUFFIX_LIST:
        file_list.extend(
            glob.glob(
                os.path.join(context[KEY_SRC][KEY_LIBRARIES], library, "*.%s" % ext)
            )
        )
...
def get_relevant_version(library):
    ...
    if library == "bokeh":
        ...
        tags = requests.get(
            url="https://api.github.com/repos/%s/tags" % LIB_DICT[library][KEY_GITHUB]
        ).json()
        try:
        ...
        if url.startswith("http"):
            ...
            if library == LIB_KATEX and fn.endswith(".css"):
                ...
                cnt = cnt.replace(
                    ...
                    "url(%s/"
                    % os.path.relpath(
                        context[KEY_OUT][KEY_FONTS], context[KEY_OUT][KEY_STYLES]
                    ),
                )
                with open(fp, "w+") as f:
                    f.write(cnt)
