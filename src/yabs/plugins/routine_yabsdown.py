...
class RendererWithMath(mistune.Renderer):
    ...
    def block_math(self, text):
        ...
        return self.options[KEY_TEMPLATES][KEY_FORMULA].render(
            formula=self.options[KEY_MATH](text), number=self.counter_dict[KEY_FORMULA]
        )
    ...
    options.update(
        {
            KEY_IMAGES: os.path.relpath(
                context[KEY_OUT][KEY_IMAGES], context[KEY_OUT][KEY_ROOT]
            )
        }
    )

    return MarkdownWithMath(renderer=RendererWithMath(**options))
